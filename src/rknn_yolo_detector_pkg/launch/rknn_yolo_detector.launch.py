...
def generate_launch_description() -> LaunchDescription:
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument("hud_enabled", default_value="true", description="[显示] 是否在 debug 图中绘制实时 FPS/时延 HUD。"),
        DeclareLaunchArgument("hud_stats_window_sec", default_value="1.0", description="[显示] FPS、DROP 和耗时统计窗口（秒）。"),
        DeclareLaunchArgument("system_metrics_period_sec", default_value="1.0", description="[显示] CPU/内存/温度/NPU 指标采样周期（秒）。"),
        DeclareLaunchArgument("hud_show_system_metrics", default_value="true", description="[显示] 是否展示 CPU、内存和温度。"),
        DeclareLaunchArgument("hud_show_npu_metrics", default_value="true", description="[显示] 是否展示 NPU load 与频率；不可用时显示 N/A。"),
        Node(
