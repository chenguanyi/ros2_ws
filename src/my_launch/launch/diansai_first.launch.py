...
#
# 任务流程：起飞 -> 固定取物点 -> 底部相机红色方块视觉对准 -> 锁定当前 XY 下降取物
# -> 最多 3 次取物检查/重试 -> 搬运到终点 -> 放下铁块 -> 下降完成。
# =============================================================================

TAKEOFF_POINT = (0.0, 0.0, 80.0, 0.0)
PICKUP_POINT = (0.0, 135.0, 80.0, 0.0)
ENDPOINT = (60.0, -120.0, 80.0, 0.0)

ACTION_HEIGHT_CM = 23.0
...
WEB_BRIDGE_DELIVERY_TOPIC = "/web/relay_delivery"
WEB_BRIDGE_STATUS_TOPIC = "/web/relay_status"
WAIT_FOR_WEB_COMMAND = True
...
# 高度使用面阵激光（直连香橙派 USB/串口），通过 laser_array_ground 发布 /laser_array/ground_height。
# uart_to_stm32 节点订阅后转发为 /height。
...
def _diansai_task_params():
    params = {
        ...
        "mission_command_topic": WEB_BRIDGE_DELIVERY_TOPIC,
        "relay_status_topic": WEB_BRIDGE_STATUS_TOPIC,
        "wait_for_mission_command": WAIT_FOR_WEB_COMMAND,
        ...
        TimerAction(
            ...
            actions=[
                Node(
                    ...
                    executable="relay_http_bridge.py",
                    name="diansai_first_relay_http_bridge",
                    output="screen",
                    parameters=[{
                        "host": WEB_BRIDGE_HOST,
                        "port": WEB_BRIDGE_PORT,
                        "relay_delivery_topic": WEB_BRIDGE_DELIVERY_TOPIC,
                        "relay_status_topic": WEB_BRIDGE_STATUS_TOPIC,
